...
(function (window) {
      ...
      onMouseMoveFn = function (ev) {
        requestAnimationFrame(function () {
          ...
          const mousepos = getMousePos(ev),
            rotX =
              ((2 * self.options.tilt.maxRotationX) / self.win.height) *
                mousepos.y -
              self.options.tilt.maxRotationX,
            rotY =
              ((2 * self.options.tilt.maxRotationY) / self.win.width) *
                mousepos.x -
              self.options.tilt.maxRotationY,
            transX =
              ((2 * self.options.tilt.maxTranslationX) / self.win.width) *
                mousepos.x -
              self.options.tilt.maxTranslationX,
            transY =
              ((2 * self.options.tilt.maxTranslationY) / self.win.height) *
                mousepos.y -
              self.options.tilt.maxTranslationY;
  ...
  GlitchFx.prototype.glitch = function () {
    ...
    this.glitchTimeout = setTimeout(function () {
      ...
      this.options.glitchStart.min,
      this.options.glitchStart.max
    ));
  };
  ...
  GlitchFx.prototype._glitchState = function (callback) {
    ...
    if (this.iteration < this.options.glitchTotalIterations) {
      this.glitchStateTimeout = setTimeout(function () {
        ...
        this.options.glitchState.min,
        this.options.glitchState.max
      ));
    } else {
      callback.call();
    }
  ...
  const DOM = {},
    is3DBuggy = navigator.userAgent.indexOf("Firefox") > 0;
  let pm, gfx;
